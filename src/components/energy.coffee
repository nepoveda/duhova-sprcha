React    = require('react')

EnergyScreen = ->
  <div className="pages">
    <h1>ENERGETICKÉ ČIŠTĚNÍ PROSTOR</h1>
    <h2> Co je to? </h2>
    <p>Energetické čištění prostor je čištění pomocí energií, vykuřovadel, malování či

kyvadla. Záleží na náročnosti problému, kterou metodu zvolíme. </p>
    <p>Slyšeli jste někdy o toulavých duších? Pojmy jako poltergeist, paranormální jevy,

duchové, negativní či stagnující energie, cizí bytosti? Toto nám rozhodně není cizí. </p>
    <p>Každé místo má svou minulost a v některých mohou zůstat energetické otisky.

Pokud jsou to otisky negativní, mohou ovlivňovat kvalitu našeho přítomného

života. </p>
    <p> Naším cílem je pročistit Váš byt či dům, rychle a efektivně od všeho, co by Vám

mohlo narušovat klid a pohodu. </p>
    <h2> Jak to poznám? </h2>
    <p> Špatně se Vám spí? </p>
    <p>Slyšíte zvuky, které logicky není možné, aby z onoho místa vycházeli? </p>
    <p> Vidíte stíny? Máte pocity úzkosti? </p>
    <p> Chcete udělat rekonstrukci prostorů a neustále jsou Vám kladeny překážky? </p>
    <h2> Ceník </h2>
    <p> 1 hod/1000,- Kč… každá další započatá hodina 500,-Kč

V případě delšího dojezdu si účtujeme cestovné dle dohody. </p>


  </div>

module.exports = { EnergyScreen }
